...
class ConnectionResolver {
  ...
  Future<ConnectionParams?> _resolveInDiscovery(
      IContext? context, ConnectionParams connection) async {
    ...
    if (discoveries.isEmpty) {
      var err = ReferenceException(context, discoveryDescriptor);
      throw err;
    }
    ...
    for (var discovery in discoveries) {
      ...
      var result =
          key != null ? await discoveryTyped.resolveOne(context, key) : null;
      if (result != null) {
        firstResult = result;
        break;
      }
  ...
  ///
  /// See [IDiscovery]
  Future<ConnectionParams?> resolve(IContext? context) async {
    ...
    for (var connection in connections) {
      var result = await _resolveInDiscovery(context, connection);
      if (result != null) {
        firstResult = result;
        break;
      }
  ...
  Future<List<ConnectionParams>> _resolveAllInDiscovery(
    ...
    for (var discovery in discoveries) {
      IDiscovery discoveryTyped = discovery;
      var result =
          key != null ? await discoveryTyped.resolveAll(context, key) : null;
  ...
  ///
  /// See [IDiscovery]
  Future<List<ConnectionParams>> resolveAll(IContext? context) async {
    var resolved = <ConnectionParams>[];
    ...
    for (var connection in toResolve) {
      var result = await _resolveAllInDiscovery(context, connection);
  ...
  Future<ConnectionParams> register(
      IContext? context, ConnectionParams connection) async {
    var result = await _registerInDiscovery(context, connection);
    if (result) {
      _connections.add(connection);
    }
    return connection;
