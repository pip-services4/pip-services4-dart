...
class CredentialResolver {
  ...
  Future<CredentialParams?> _lookupInStores(
      IContext? context, CredentialParams credential) async {
    ...
    if (components.isEmpty) {
      throw ReferenceException(context, storeDescriptor);
    }

    CredentialParams? firstResult;
    for (var component in components) {
      ICredentialStore store = component;
      var result = await store.lookup(context, key);
      if (result != null) {
        firstResult = result;
        break;
      }
  ...
  /// Throw error
  Future<CredentialParams?> lookup(IContext? context) async {
    ...
    for (var credential in lookupCredentials) {
      var result = await _lookupInStores(context, credential);
      if (result != null) {
        firstResult = result;
        break;
      }
