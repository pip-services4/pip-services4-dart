...
import '../connect/IMqttMessageListener.dart';
import '../connect/MqttConnection.dart';
...
///
/// See [MessageQueue](https://pub.dev/documentation/pip_services4_messaging/latest/pip_services4_messaging/MessageQueue-class.html)
/// See [MessagingCapabilities](https://pub.dev/documentation/pip_services4_messaging/latest/pip_services4_messaging/MessagingCapabilities-class.html)
///
/// ### Example ###
/// ```dart
...
///         ...
///
///     await queue.send(Context.fromTraceId('123'), MessageEnvelope(null, 'mymessage', 'ABC'));
///
...
class MqttMessageQueue extends MessageQueue
  ...
  final _dependencyResolver =
      DependencyResolver(MqttMessageQueue._defaultConfig);
  ...
  Future open(IContext? context) async {
    ...
    if (_localConnection != null && _localConnection != false) {
      await _connection!.open(context);
    }
  ...
  Future close(IContext? context) async {
    ...
    if (_localConnection) {
      await _connection!.close(context);
    }
  ...
  Future subscribe(IContext? context) async {
    if (_subscribed) {
      return;
    }
  ...
  void onMessage(String topic, String data, packet) {
    ...
    _logger.debug(Context.fromTraceId(message.trace_id ?? ''),
        'Received message %s via %s', [message, getName()]);
  ...
  Future<MessageEnvelope?> peek(IContext? context) async {
    ...
    if (message != null) {
      _logger.trace(Context.fromTraceId(message.trace_id ?? ''),
          'Peeked message %s on %s', [message, getName()]);
    }
  ...
  Future send(IContext? context, MessageEnvelope message) async {
    ...
    _logger.debug(Context.fromTraceId(message.trace_id ?? ''),
        'Sent message %s via %s', [message.toString(), toString()]);
  ...
  void sendMessageToReceiver(
    ...
    var context = Context.fromTraceId(message?.trace_id ?? '');
    if (message == null || receiver == null) {
      _logger.warn(context, 'MQTT message was skipped.');
      return;
    }

    await _receiver!.receiveMessage(message, this).catchError((err) =>
        {_logger.error(context, err, 'Failed to process the message')});
  }
  ...
  ///
  ///See [IMessageReceiver](https://pub.dev/documentation/pip_services4_messaging/latest/pip_services4_messaging/IMessageReceiver-class.html)
  ///See [receive]
  @override
  void listen(IContext? context, IMessageReceiver receiver) async {
    ...
    // Subscribe to topic if needed
    await subscribe(context).then((value) {
