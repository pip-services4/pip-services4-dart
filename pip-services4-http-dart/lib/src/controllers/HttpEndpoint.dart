...
class HttpEndpoint implements IOpenable, IConfigurable, IReferenceable {
  ...
  List<String> _allowedHeaders = ['trace_id'];
  ...
  ///
  /// See [IReferences](https://pub.dev/documentation/pip_services4_components/latest/pip_services4_components/IReferences-class.html) (in the PipServices 'Commons' package)
  @override
  ...
  Future open(IContext? context) async {
    ...
    try {
      ...
      _logger.debug(context, 'Opened REST service at %s', [_uri]);
    } catch (ex) {
      _server = null;
      _app = null;
  ...
  Future close(IContext? context) async {
    if (_server != null) {
      ...
      try {
        ...
        _logger.debug(context, 'Closed REST service at %s', [_uri]);
      } catch (ex) {
        _logger.warn(context, 'Failed while closing REST service: %s', [ex]);
        rethrow;
      }
