...
class IdentifiableJsonMySqlPersistence<T extends IIdentifiable<K>, K>
  ...
  void ensureTable_({String idType = 'VARCHAR(32)', String dataType = 'JSON'}) {
    if (schemaName_ != null) {
      var query =
          "CREATE SCHEMA IF NOT EXISTS ${quoteIdentifier_(schemaName_)}";
      ensureSchema_(query);
    }
    var query =
        "CREATE TABLE IF NOT EXISTS ${quotedTableName_()} (`id` $idType PRIMARY KEY, `data` $dataType)";
    ensureSchema_(query);
  }
  ...
  Future<T?> updatePartially(
    ...
    var query =
        "UPDATE ${quotedTableName_()} SET `data`=JSON_MERGE_PATCH(data,?) WHERE id=?";
