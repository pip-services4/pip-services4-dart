...
class MySqlPersistence<T>
  ...
  Future close(IContext? context) async {
    ...
    if (this._localConnection) {
      await this.connection_!.close(context);
    }
  ...
  Future<void> createSchema_(IContext? context) async {
    ...
    if (this._schemaStatements == null || this._schemaStatements.isEmpty) {
      return;
    }
    ...
    var exist = res.isNotEmpty;

    // If table already exists then exit
    if (exist) return;

    logger_.debug(context,
        'Table ${this.tableName_} does not exist. Creating database objects...');
  ...
  Future<int> getCountByFilter_(IContext? context, String? filter) async {
    ...
    logger_.trace(context, "Counted %d items in %s", [count, this.tableName_]);
  ...
  Future<List<T>> getListByFilter_(
    ...
    logger_
        .trace(context, "Retrieved %d from %s", [res.length, this.tableName_]);
  ...
  Future<T?> getOneRandom_(IContext? context, String? filter) async {
    ...
    if (mapItem == null) {
      this.logger_.trace(
          context, "Random item wasn't found from %s", [this.tableName_]);
    } else {
      this
          .logger_
          .trace(context, "Retrieved random item from %s", [this.tableName_]);
    }
  ...
  Future<T?> create(IContext? context, T item) async {
    ...
    var query =
        "INSERT INTO ${this.quotedTableName_()} ($columns) VALUES ($params)";

    var res = await client_!.query(query, values);

    logger_.trace(context, "Created in %s with id = %s",
