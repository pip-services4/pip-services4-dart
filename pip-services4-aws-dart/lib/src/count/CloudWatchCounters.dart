...
class CloudWatchCounters extends CachedCounters
  ...
  Future save(List<Counter> counters) async {
    ...
    for (var counter in counters) {
      ...
      if (data.length >= 20) {
        try {
          await _service?.putMetricData(
              metricData: data, namespace: _source ?? '');
        } catch (ex) {
          _logger.error(null, ApplicationException().wrap(ex),
              'cloudwatch_counters', ['putMetricData error']);
        }
        data = <MetricDatum>[];
      }
    }
    ...
    if (data.isNotEmpty) {
      try {
        ...
        _logger.error(null, ApplicationException().wrap(err),
            'cloudwatch_counters', ['putMetricData error']);
      }
    }
  }
