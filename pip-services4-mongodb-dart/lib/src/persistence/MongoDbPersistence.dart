...
class MongoDbPersistence<T>
  ...
  Future open(IContext? context) async {
    ...
    try {
      ...
      for (var index in _indexes) {
        ...
        logger.debug(context, 'Created index %s for collection %s',
    ...
    _opened = true;
    collection = coll;
    logger.debug(context, 'Connected to mongodb database %s, collection %s',
  ...
  Future<DataPage<T>> getPageByFilterEx(
    ...
    logger
        .trace(context, 'Retrieved %d from %s', [items.length, collectionName]);
    if (pagingEnabled) {
  ...
  /// Throw error
  Future<List<T>> getListByFilterEx(IContext? context,
  ...
  Future deleteByFilterEx(
      IContext? context, Map<String, dynamic> filter) async {
    var result = await collection?.remove(filter);
    if (result != null && result['ok'] == 1.0) {
      var count = result['n'] ?? 0;
      logger
          .trace(context, 'Deleted %d items from %s', [count, collectionName]);
    }
  }
  ...
  Future<int> getCountByFilterEx(
    ...
    logger.trace(context, 'Find %d items in %s', [count, collectionName]);
    return count;
  }
