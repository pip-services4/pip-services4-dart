...
/// ### References ###
///
/// - \*:logger:\*:\*:1.0           (optional) [ILogger](https://pub.dev/documentation/pip_services4_observability/latest/pip_services4_observability/ILogger-class.html) components to pass log messages components to pass log messages
...
class IdentifiableMongoDbPersistence<T extends IIdentifiable<K>, K>
  ...
  Future<List<T>> getListByIds(IContext? context, List<K> ids) async {
    ...
    return getListByFilterEx(context, filter, null);
  }
  ...
  Future<T?> getOneById(IContext? context, K? id) async {
    ...
    if (item == null) {
      logger.trace(
          context, 'Nothing found from %s with id = %s', [collectionName, id]);
      return null;
    }
    logger
        .trace(context, 'Retrieved from %s with id = %s', [collectionName, id]);
  ...
  Future<T?> create(IContext? context, T? item) async {
    ...
    if (result != null && result.ok == 1.0) {
      logger.trace(context, 'Created in %s with id = %s',
  ...
  Future<T?> set(IContext? context, T? item) async {
    ...
    if (result != null && result.ok == 1.0) {
      logger
          .trace(context, 'Set in %s with id = %s', [collectionName, item.id]);
  ...
  Future<T?> update(IContext? context, T? item) async {
    ...
    if (result != null) {
      logger.trace(
          context, 'Updated in %s with id = %s', [collectionName, item.id]);
  ...
  Future<T?> updatePartially(
    ...
    if (result != null && result.ok == 1.0) {
      logger.trace(context, 'Updated partially in %s with id = %s',
          [collectionName, id]);

      return await getOneById(context, id);
    }
    return null;
  }
  ...
  Future<T?> deleteById(IContext? context, K? id) async {
    ...
    var oldItem = await getOneById(context, id);
    var result = await collection?.remove(filter);
    if (result != null && result['ok'] == 1.0) {
      logger
          .trace(context, 'Deleted from %s with id = %s', [collectionName, id]);
  ...
  /// Throws error
  Future deleteByIds(IContext? context, List<K> ids) async {
    var filter = {
      '_id': {r'$in': ids}
    };
    return deleteByFilterEx(context, filter);
  }
}
