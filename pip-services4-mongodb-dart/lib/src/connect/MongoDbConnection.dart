...
import 'MongoDbConnectionResolver.dart';
...
class MongoDbConnection implements IReferenceable, IConfigurable, IOpenable {
  ...
  Future open(IContext? context) async {
    ...
    logger.debug(context, 'Connecting to mongodb');
    ...
    try {
      ...
      var credential = await connectionResolver.credentialResolver
          .lookup(Context.fromTraceId(''));
  ...
  Future close(IContext? context) async {
    ...
    try {
      ...
      databaseName = null;
      logger.debug(
          context, 'Disconnected from mongodb database %s', [databaseName]);
    } catch (err) {
      throw ConnectionException(
              context != null ? ContextResolver.getTraceId(context) : null,
              'DISCONNECT_FAILED',
